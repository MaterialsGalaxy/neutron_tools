...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v",
        "--equation-variables",
        help="equation constraint variables list",
        type=str,
        nargs="+",
    )
    parser.add_argument(
        ...
        help="equation constraint coefficient list",
        type=str,
        nargs="+",
    )
    parser.add_argument(
        "-t",
        "--equation-totals",
        help="equation constriant total",
        type=float,
        nargs="+",
    )
    parser.add_argument(
        ...
        help="equivalence constraint variable list",
        type=str,
        nargs="+",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-n", "--num-cycles", help="Number of refinement cycles", type=int, default=5
    )
    ...
    if args.equation_variables is not None:
        for i in range(len(args.equation_variables)):
            args.equation_variables[i] = [
                s.strip() for s in args.equation_variables[i].split(",")
            ]

    if args.equation_coefficients is not None:
        for i in range(len(args.equation_coefficients)):
            args.equation_coefficients[i] = [
                float(s) for s in args.equation_coefficients[i].split(",")
            ]
    else:
        for i in range(len(args.equation_variables)):
    ...
    if args.equivalence_variables is not None:
        for i in range(len(args.equivalence_variables)):
            args.equivalence_variables[i] = [
                s.strip() for s in args.equivalence_variables[i].split(",")
            ]

    if args.equivalence_coefficients is not None:
        for i in range(len(args.equivalence_coefficients)):
            args.equivalence_coefficients[i] = [
                float(s) for s in args.equivalence_coefficients[i].split(",")
            ]
    else:
        for i in range(len(args.equivalence_coefficients)):
            args.equivalence_coefficients[i] = [1.0] * len(
                args.equivalence_variables[i]
            )
